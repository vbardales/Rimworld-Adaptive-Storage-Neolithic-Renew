...
// This mod is defs, patches and textures only: no assembly. A DLL in Mod/ would be a regression, and a Mod/ without its defs an empty upload.
await stat(join(modPath, 'Defs'));
if ((await readdir(modPath)).includes('Assemblies')) throw new Error('Mod/Assemblies exists, but this mod ships no assembly');
