...
remaining:
  - resolved 2026-09-21 (preTest -> done): the Pickle suite is written in tests/Pickle (32 scenarios, 5 features, 18 @review captures, no step assembly), scope justified in tests/Pickle/README.md and TESTING.md. Run three times in the WSL the same day, at 20 scenarios: first 19 played of 19 written (16 passed, 1 failed on a defect of the suite, 2 skipped as @wip); after the fix, in English, 20 of 20 (18 passed, 0 failed, 2 skipped as @wip), exitReason passed; in French with -IncludeWip, 20 of 20 (20 passed, 0 skipped), exitReason passed. Widened afterwards to 32 scenarios and 18 @review captures (research projects and their window, blueprints and frames, French descriptions and inspect pane), not yet played. Results: tests/pickle-run-2026-09-21/, -second/, -french/.
  - unverified (done -> tested): replay the widened suite in English and in French (-IncludeWip), since twelve scenarios added on 2026-09-21 have never run; their six assumptions are listed at the end of tests/Pickle/README.md. Then what the suite still does not reach. The French pass showed the buildings named correctly in developer mode (Panier en bois, Amas de granite, Socle en vaccolithe...) but not the inspect-pane descriptions, the research tab, blueprints and frames, the piles, shelf and bundle names, or a third-party stone with the Harmony fallback. All 16 @review captures were looked at in English across two runs and the French ones for their labels only; the framing is wide (whole colony in frame), and a person still has to judge them. The manual scenarios 4b, 8, 9 and 10, the logs beyond the Pickle report, a new game and an existing save are not done.
...
The Pickle suite has run green in English and French. What remains for `tested`: the manual scenarios the suite does not cover (TESTING.md 4b, 8, 9, 10), the French and English interface beyond the captured labels (inspect-pane descriptions, research tab, blueprints and frames) in developer mode, the logs of a real session, a new game and an existing save, and a person's judgement of the 16 captures. Fix what turns up and rerun the affected checks. Settings and shortcut checks are not applicable.
