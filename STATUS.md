...
remaining:
  - 2026-09-22 Workshop gallery: the current integrated tree's six capture scenarios passed 6/6 in the Nelim screenshot studio, at maximum zoom over the upper-right orange part of its emblem (archived report `pickle-reports-archive/0922-2226`). All six PNGs were copied, deterministically cropped and inspected for legibility in `Art/WorkshopScreenshots`. The owner still needs to approve the final images and upload them to Steam. This presentation pass does not replace the full English/French/Russian gameplay suite.
  ...
  - unverified (done -> tested), 2026-09-22 upstream integration: the mod content was replaced with bbradson/Adaptive-Storage-Neolithic-Module main at 2bc3fe4 plus the 1.6/case/French corrections proposed upstream in PR #4. The former Workshop-based generator architecture, Harmony translation DLL and its runtime evidence no longer describe the delivered files. Static checks pass (Test-Mod.ps1 175; Test-InstalledTranslations.ps1 123), and the Pickle step assembly compiles with new stuff-aware steps, but this revision has not run in RimWorld. Run the revised Pickle suite in English, French and Russian, including Odyssey and KExtraStone passes, and review every @review capture.
...
untracked `docs/runs/2026-09-13-audits.md`. This audit adds this section and
