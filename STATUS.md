...
remaining:
  ...
  - publishing (tested -> prepublished), state 2026-09-24: version `1.1.0` is prepared on the tested tree. The manual workflow `.github/workflows/publish-tag.yml` (no assembly, so no build; adapted from Skill Icons) uploads `Mod/` to item `3806101377` with the change note under `### 1.1.0` of PUBLICATION.md and never sends description, title, preview or visibility; the CI creates tag `v1.1.0` and its release after a successful upload, nothing by hand. Dry-run passed: run 35966977983, commit `0ab6a586cf7c0182821ad3da2f23aa71f6d8d2ab`, 2026-09-24, mode `dry-run`, environment `release-dry-run`; log read: the commit is on `main`, tag `v1.1.0` does not exist, the `## [1.1.0]` section is found, 205 files staged (2.62 MB: About 4, Defs 21, Languages 6, Patches 1, Textures 171, LICENSE, ATTRIBUTION.md; no assembly, no README), target item and packageId match `Mod/About`, generated `workshop.vdf` carries only the change note, "nothing was sent to Steam". The dry-run cannot show the upload itself. Checked before the run: `steam-production` has one required reviewer and holds `STEAM_USERNAME` and `STEAM_CONFIG_VDF_B64` (names only). NEXT, Virginie only: launch `publish` for that exact SHA with `Rimworld-Release-Admin/scripts/dispatch-publish.sh vbardales/Rimworld-Adaptive-Storage-Neolithic-Renew publish-tag.yml 0ab6a586cf7c0182821ad3da2f23aa71f6d8d2ab 1.1.0` and approve `steam-production`; no session runs or approves it. Any commit that touches `Mod/`, PUBLICATION.md or CHANGELOG.md before then needs a new dry-run and a new SHA. STATUS.md and docs changes do not (the workflow reads only the pinned commit). AFTER the upload: check the item's update time, payload and change note on the public page, then record it here. NOT DONE by design: the adult-content boxes, pasting the description, approving and uploading the six images, the thanks messages once the item is public; the item stays private until then.
