...
remaining:
  - resolved 2026-09-21 (preTest -> done): the Pickle suite is written in tests/Pickle (30 scenarios, 5 features, 19 @review captures, no step assembly), scope justified in tests/Pickle/README.md and TESTING.md. It was played seven times in the WSL the same day. At 20 scenarios: 19 played of 19 (16 passed, 1 failed on a defect of the suite, 2 skipped as @wip), then English 20 of 20 (18 passed), then French 20 of 20 (all passed), exitReason passed each time. Widened afterwards, four passes: English 32 of 32 with 3 failed (my wrong assumption that Pickle sees the implied blueprint defs), then English 31 of 31 and French 31 of 31, 22 passed each with the 9 @wip skipped, exitReason passed; then the French feature 05 alone, 9 of 9 with 6 passed and 3 failed (two ambiguous defNames and a guessed blueprint label, all the suite's own). Every failure so far was a defect of the suite, none of the mod. 05 was rewritten (8 scenarios) and not replayed. Results: tests/pickle-run-2026-09-21/ and its -second, -french, -widened-en, -widened-en2, -widened-fr, -widened-fr-wip siblings.
  - unverified (done -> tested): replay the rewritten French feature 05 (-Language French -Filter 05-french-names.feature -IncludeWip; the wrapper refuses -IncludeWip alone). What the passes showed in French: the blueprint of a generated building reads "Grand pot en granite (plan)", so the mod's Harmony hook ran between the language injection and the implied defs, and the inspect pane, the storage tab ("Stockage") and the filter rows are French with no accented fallback text; the research window's framework tab reads "Stockage". Not reached by any pass: the descriptions in the inspect pane, the research projects inside their tab, frames, the piles, shelf and bundle names beyond the mouse-over lines, and a third-party stone with the Harmony fallback. All 19 @review captures were looked at at least once, partly as crops; the framing is wide (whole colony in frame) and a person still has to judge them. The manual scenarios 4b, 8, 9 and 10, the logs of a real session, a new game and an existing save are not done.
