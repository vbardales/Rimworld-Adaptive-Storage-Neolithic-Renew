...
remaining:
  - resolved 2026-09-21 (preTest -> done): the Pickle suite is written in tests/Pickle (20 scenarios, 5 features, 16 @review captures, no step assembly), scope justified in tests/Pickle/README.md and TESTING.md. First run the same day, in the WSL: 19 played of 19 written, 16 passed, 1 failed, 2 skipped (@wip), exitReason failed; the failure was a defect of the suite (an ambiguous defName), fixed by naming the def type, and the rewritten scenarios have not been played. Results: tests/pickle-run-2026-09-21/.
  - unverified (done -> tested): replay the suite in English, then in French with -IncludeWip (compare scenarios played with the 20 written), including the rewritten 01 and the never-played 05. The six assumptions of the first run are settled (tests/Pickle/README.md). 15 of 16 @review captures were looked at, in this session and partly as crops: the basket fills up, the chunk stack swaps sprite at one, two and six chunks, the marble stack is lighter than the granite one, containers redraw after a reload. The framing is wide (whole colony in frame); a person still has to judge them and the fabric-basket-empty capture is unseen.
