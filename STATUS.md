...
remaining:
  ...
  - unverified (done -> tested): finish the features written on 2026-09-21. Seen working in a running game: TESTING.md 4b in French, beside [K]Extra Stone: the generated pot, plinth and chunk stack of the stone Kura_Andesite read "grand pot (andesite chunk)", "socle (andesite chunk)", "amas de blocs (andesite chunk)" (the Harmony fallback), and the blueprint of the pot reads "Grand pot (andesite chunk) (plan)" (tests/pickle-run-2026-09-21-stones-fr/). Also seen working: 07 (Russian on the WSL's case-sensitive ext4, TESTING.md 8), 5 of 5 passed (tests/pickle-run-2026-09-21-russian/). Not played: 06 (the same stone in English; the "stones" pass began but stopped before it, exitReason in-progress, cause not established), queued again; the suite is 41 scenarios, 22 @review captures. The research-tab click (English and French) was tried and dropped: Pickle recorded no button tag for the tab, so the two projects inside their tab stay a person's to look at. What the earlier passes showed in French (blueprint "Grand pot en granite (plan)", inspect pane, "Stockage" tab, no accented fallback) holds. All earlier @review captures were looked at at least once; the framing is wide and a person still has to judge them. The logs of a real session are not done.
